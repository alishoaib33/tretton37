  ...
  function ManageParamChange(e) {
      ...
      if(e.target.name == '_sort'){
         ...
         let sortOrder = e.target.value.split("&")['1']
          ...
          var  param = (e.target.name == 'searchBtn' ? e.target.parentElement.firstElementChild.name:e.target.name)
          var value = (e.target.name == 'searchBtn' ? e.target.parentElement.firstElementChild.value:e.target.value)
  ...
  return (
      ...
      <div class="row">
        { profiles.length>0 ? profiles.map((profile, index) => {
            return <Profile key={index+'-'+profile.phoneNumber} profile={profile}  linkedinUrl={linkedinUrl} githubUrl={githubUrl} twitterUrl={twitterUrl} />
       }) :  <NoProfileFound />}
       </div>
        {totalItemsReturned < 8  ?'' :  <ProfilesPagination page={page} setPage={setPage} hasNextPage={hasNextPage}/>}
    </div>
  )
}
